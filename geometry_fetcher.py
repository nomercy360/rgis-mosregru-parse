...
async def fetch_all_geometries(input_file, output_file, max_concurrent_requests):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(geometries, f, ensure_ascii=False, indent=2)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Fetch geometry data for card IDs')
    parser.add_argument('--input', type=str, default='data.json',
                        help='Input JSON file with geometry_ids (default: data.json)')
    parser.add_argument('--output', type=str, default='geometries.json',
                        help='Output JSON file for geometries (default: geometries.json)')
